...
%opening
\title{FluxEngine v4.1.0: Changes and User Guide}
...
If you're interested in modifying, contributing to, or extending FluxEngine, you can install the package locally and its dependencies, including dev-dependencies, by cloning the git repository and running \texttt{pip install -e ".[dev]"} from the repository root directory. It's recommended to do this in a fresh virtual environment to avoid package clashes. You should only need to do this if you intend to modify the core code.

In FluxEngine v3.0 we have made changes to make it easier for people to modify and extend the certain aspects of FluxEngine's functionality without modifying the core code. The following sections provide some background information on how to do this. This is intended for users who are comfortable programming in Python.
...
The gas transfer velocity calculation is fully customisable and is is possible to write custom 'functor' classes to add to the built-in parameterisations available. There is an example of how to do this by extending a base k-parameterisation class (look for \texttt{k\_example} in \texttt{fluxengine{\char`\\}core{\char`\\}rate\_parameterisation.py}). Custom k-parameterisations can expose parameters to change their behaviour. These parameters should be passed as arguments to their \texttt{\_\_init\_\_} function, and the values must then be defined as a configuration variable in the config file (see section \ref{adding_custom_config_variables} for how to add custom configuration variables). Important note: the custom configuration variables must exactly match the argument names use by k-parameterisation implementations \texttt{\_\_init\_\_} function. It is advisable to give these a prefix unique to the custom k-parameterisation, even if this is a little verbose, as this minimised the risk of name clashes. E.g., something like \texttt{MyProjectNamesKParam\_scaling\_coefficient = 2.0} is generally better than \texttt{scaling\_coefficient = 2.0}.

In older version of FluxEngine, custom k-parameterisations were added directly to the parameterisations to the \texttt{fluxengine{\char`\\}core{\char`\\}rate\_parameterisation.py} file, thereby making them globally available to any project. As of FluxEngine v4.0.3, the new recommended way to add parameterisations is to implement them in a separate file which can be stored with your other project files / config files etc. You can then pass the path to this file as an argument when running FluxEngine and the contents of the file will be executed, and any parameterisations will be available for use in configuration files. The advantages to this are 1) Parameterisations can be stored with other project-specific files, such as config files, and therefore moved around with them. 2) Custom parameterisations aren't lost when updating FluxEngine. And 3) parameterisations can be easily shared, e-mails or attached as supplementary material to manuscripts.

Here is an example of a simple (but nonsensible) implementation of a custom k-parameterisation, which is used for testing.
...
Since the above example gas transfer parameterisation requires one parameter, \texttt{k\_CustomGtvTest\_some\_parameter}, this parameter must be defined in the configuration file, for example by including a line like this \texttt{k\_CustomGtvTest\_some\_parameter = 2.5}. FluxEngine will automatically extract the parameter value and use it to construct the gas transfer parameterisation functor.
...
\subsubsection{Adding configuration variables} \label{adding_custom_config_variables}
