    ...
    if configParametersElement != None:
        for element in configParametersElement:
            try:
                        ...
                        for child in element:
                            if child.tag == "Option":
                                try:
                                    ...
                                    print(
                                        "%s: Invalid multioption child 'Option' element in %s. 'str' and 'value' attributes must be specified." % (
                                        function, element.attrib["name"]))
                                    print(e.args)
                ...
                print(
                    "%s: 'name' or 'type' attribute missing from Variable element in settings xml file. Specified settings file is invalid: %s" % (
    ...
    for varName in standardVars:
        ...
        if metadata[varName]["type"] == "multioption":
            if configVariables[varName] not in metadata[varName]["options"]:
                raise ValueError("%s: Invalid option specified in config file for %s. Valid options are: %s" % (
                function, varName, str(list(metadata[varName]["options"].keys()))))
            configVariables[varName + "_name"] = configVariables[varName]  # name is useful for writing messages to user
            configVariables[varName] = metadata[varName]["options"][
                configVariables[varName]]  # Overwrite with integer option
        ...
        # Switch: convert 'yes' and 'no' strings to True and False.
            ...
            if configVariables[varName].lower() == "yes":
                ...
                raise ValueError(
                    "%s: Invalid value for %s in config file. Accepted values are 'yes' or 'no' but found '%s'." % (
        ...
        # Float: string to float.
            ...
            try:
                ...
                print("%s: Config variable '%s' requires a decimal / floating point number. Got %s instead." % (
                function, varName, configVariables[varName]))
    ...
    for varName in standardVars:
        ...
        if metadata[varName]["type"] == "DataLayerPath":
            if varName[0:-4] + "prod" not in configVariables:
                raise ValueError(
                    "%s: DataLayer path is specified (%s) but there is no corresponding prod. '%s' must be also be defined in the configuration file." % (
                    function, varName, varName[0:-4] + "prod"))
...
def substitute_tokens(inputStr, curDatetime):
    ...
    # <DDD> day of the year
    cumulativeDaysByMonth = [0] + list(cumsum([calendar.monthrange(year, m)[1] for m in range(1,
                                                                                              12)]))  # Cumulative days in each month of the year (accounting for leap years)
    outputStr = outputStr.replace("<DDD>", "%03d" % (day + cumulativeDaysByMonth[
        month - 1]))  # Day number in year, starting at 1 (up to 365 or 366 on leap years)
...
def create_run_parameters(configVariables, varMetadata, curTimePoint, executionCount, processTimeStr, configFile,
                          processIndicatorLayersOff, previousRunParams):
    function = inspect.stack()[0][1] + ", " + inspect.stack()[0][3]
    ...
    for varName in configVariables:
        ...
        if varName in varMetadata:
            if varMetadata[varName]["type"] == "DataLayerPath":
                pathGlob = substitute_tokens(configVariables[varName],
                                             curTimePoint)  # substitute time tokens into the path/glob
                pathGlob = path.abspath(
                    path.expanduser(pathGlob))  # Expand to absolute path only AFTER substituting tokens
                ...
                try:
                    ...
                    matches = [match for match in matches if match.rsplit('/', 1)[-1][
                        0] != '.']  # Remove hidden metadata files created by MACOS on some file systems. Required if sharing folders between MACOS and LINUX/WINDOWS.
                    ...
                    if len(matches) == 1:  # Store the file path.
                        ...
                        runParams[dataLayerName + "_stddev_prod"] = configVariables[
                            dataLayerName + "_stddev_prod"] if dataLayerName + "_stddev_prod" in configVariables else None
                        runParams[dataLayerName + "_count_prod"] = configVariables[
                            dataLayerName + "_count_prod"] if dataLayerName + "_count_prod" in configVariables else None

                        if dataLayerName + "_preprocessing" in configVariables:
                            runParams[dataLayerName + "_preprocessing"] = configVariables[
                                dataLayerName + "_preprocessing"]
                        else:
        ...
        for name, obj in gtvList:
            if (name == selectedParameterisation) and (inspect.isclass(obj) == True):
                ...
                try:
                    ...
                    print(
                        "%s: Could not find a k parameterisation functor which corresponds to the specified k_parameterisation (%s). If this specified correctly in the config file?" % (
                        function, name))
                    print(e.args)
                    return None
                ...
                if issubclass(ClassHandle, k_params.KCalculationBase):
                    ...
                    initialiserArgNames = [arg for arg in inspect.getfullargspec(ClassHandle.__init__).args if
                                           arg != "self"]  # DJF - 14/11/2024 - Updated inspect.getfullargspec to the new Python call (old call now deprecated)

                    try:
                        # Create a dictionary of name:value pairs for the __init__ arguments. Assumes arguments match config file names.
                        argDict = {key: runParameters[key] for key in initialiserArgNames if
                                   key in runParameters}  # create a dictionary of arguments name:value pairs
...
def fe_obj_from_run_parameters(runParameters, metadata, processLayersOff=True, customGTVPath=None, verbose=False):
    ...
    if kFunctor != None:
        ...
        raise SystemExit(
            "Could not create the k_parameterisation object. Does the config file specifiy a valid k_parameterisation (%s is used) and specify all the required initialisers?" % (
            runParameters["k_parameterisation_name"]))
    ...
    if runParameters[
        "k_rain_nonlinear_h2012_switch"] == 1:  # and runParameters["k_parameterisation"] == 9): #9==generic_k
    ...
    for dataLayerName in inputDataLayers:
        if dataLayerName + "_infile" in runParameters:
            ...
            status = fe.add_data_layer(dataLayerName, runParameters[dataLayerName + "_infile"],
                                       ...
                                       stddevProd=runParameters[dataLayerName + "_stddev_prod"],
                                       # Note: if no stddev or count data then these are set to None
...
def run_fluxengine(configFilePath, startDate, endDate, singleRun=False, verbose=False, processLayersOff=True,
    ...
    if (pco2DirOverride != None):
        print(
            "Using optional override for pCO2w data directory. '%s' will be set to '%s' (ie overriding both directory of pco2 data and selection in the configuration file)." % (
            configVariables["pco2"], pco2DirOverride))
    ...
    if (outputDirOverride != None):
        ...
        print(
            "Using optional override for output directory, output_dir will be set to %s (overriding the configuration file value)." % (
                outputDirOverride))

    # Printing some feedback...
    if processLayersOff == True and verbose:
        print(
            "Switching off generation of processing indicator layers. This reduces the processing time by appx. 50% (switch: process_layers_off).")
        # configVariables["process_indicator_layers"] = None
    if takahashiDriver == True and verbose:
        print(
            "This is a takahashi validation run. Ensure config is the configs/takahashi09_validation.conf file supplied with FluxEngine.")
    ...
    timePoints = generate_datetime_points(startDate, endDate, deltaTime=configVariables["temporal_resolution"],
                                          singleDate=singleRun)
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for i, timePoint in enumerate(timePoints):
            ...
            try:
                ...
                runParameters = create_run_parameters(configVariables, metadata, timePoint, i, processTimeStr,
                                                      configFilePath, processLayersOff, runParameters)
        ...
        for future in futures:
            ...
            if returnCode != 0:
                ...
                print("Flux engine exited with exit code:", returnCode)
                print("%02d" % timePoint.day, calendar.month_abbr[timePoint.month], timePoint.year,
                      "%02d:%02d:%02d completed successfully.\n" % (timePoint.hour, timePoint.minute, timePoint.second))

    return (returnCode, fe)  # return code, FluxEngine object.


def process_timestep(runParameters, metadata, processLayersOff, customGTVPath, verbose=False):
    # Create fluxengine object to use runParameters
    fe = fe_obj_from_run_parameters(runParameters, metadata, processLayersOff, customGTVPath=customGTVPath,
                                    verbose=False)
