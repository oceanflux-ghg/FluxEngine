...
def GM12_kd_wind(windu10_fdata, windu10_moment2_fdata, windu10_moment3_fdata, scskin_fdata):
    ...
    valid_mask = (
            ...
            (windu10_moment2_fdata != DataLayer.missing_value) &
            (windu10_moment3_fdata != DataLayer.missing_value) &
            (scskin_fdata != DataLayer.missing_value) &
            (scskin_fdata > 0.0)
    )

    kdwind_fdata[valid_mask] = (2.2 * windu10_fdata[valid_mask] - 3.4) * np.sqrt(660.0 / scskin_fdata[valid_mask])
...
def OceanFluxGHG_k(sigma0_fdata, sig_wv_ht_fdata, windu10_fdata, windu10_moment2_fdata, sstskinC_fdata, pgas_sw_fdata,
    ...
    kinematic_fdata = np.full(dataLength, DataLayer.missing_value)
    CD_fdata = np.full(dataLength, DataLayer.missing_value)
    friction_fdata = np.full(dataLength, DataLayer.missing_value)
    kd_fdata = np.full(dataLength, DataLayer.missing_value)
    kb_fdata = np.full(dataLength, DataLayer.missing_value)

    # kinematic viscosity
    valid_kinematic = sstskinC_fdata != DataLayer.missing_value
    kinematic_fdata[valid_kinematic] = 0.00000183 * np.exp(-sstskinC_fdata[valid_kinematic] / 36.0)
    kinematic_fdata[~valid_kinematic] = DataLayer.missing_value
    ...
    CD_fdata[~valid_CD] = DataLayer.missing_value

    mask_wind_ge6 = valid_CD & (windu10_fdata >= 6.0)
    CD_fdata[mask_wind_ge6] = 0.60 + (0.070 * windu10_fdata[mask_wind_ge6])

    mask_wind_lt6 = valid_CD & (windu10_fdata < 6.0)
    CD_fdata[mask_wind_lt6] = (
            0.29 + (3.1 / windu10_fdata[mask_wind_lt6]) + (7.7 / windu10_moment2_fdata[mask_wind_lt6])
    )

    # friction velocity in units of ?
    valid_friction = (windu10_fdata != DataLayer.missing_value) & (CD_fdata != DataLayer.missing_value)
    friction_fdata[valid_friction] = np.sqrt(CD_fdata[valid_friction] * 0.001) * windu10_fdata[valid_friction]

    # kb component
    valid_kb = (
            (sig_wv_ht_fdata != DataLayer.missing_value) &
            (friction_fdata != DataLayer.missing_value) &
            (kinematic_fdata != DataLayer.missing_value) &
            (friction_fdata > 0.0) &
            (sig_wv_ht_fdata > 0.0)
    )
    kb_fdata[valid_kb] = 0.00002 * (
            ((sig_wv_ht_fdata[valid_kb] * 100.0) * friction_fdata[valid_kb] * 100 * 3600)
            / (kinematic_fdata[valid_kb] * 36000000.0)
    )
    kb_fdata[~valid_kb] = DataLayer.missing_value

    # kd component
    valid_kd = (
            (scskin_fdata != DataLayer.missing_value) &
            (sigma0_fdata != DataLayer.missing_value) &
            (scskin_fdata > 0.0)
    )

    kd_fdata[~valid_kd] = DataLayer.missing_value
    kd_fdata[valid_kd] = (
            ((2100.0 / (10.0 ** (sigma0_fdata[valid_kd] / 10.0)) ** 2) + 0.1) *
            (scskin_fdata[valid_kd] / 600.0) ** -0.5
    )
    ...
    #     # wind drag coefficient
    #     # algorithm is only value for a wind speed of up to 26 ms^-1
    #     if ((windu10_fdata[i] != DataLayer.missing_value) and (
    #             windu10_moment2_fdata[i] != DataLayer.missing_value) and (windu10_fdata[i] < 26.0) and (
    #             windu10_fdata[i] > 0.0)):
    ...
    #
    #     # kb component
    #     if ((sig_wv_ht_fdata[i] != DataLayer.missing_value) and (friction_fdata[i] != DataLayer.missing_value) and (
    #             kinematic_fdata[i] != DataLayer.missing_value) and (friction_fdata[i] > 0.0) and (
    #             sig_wv_ht_fdata[i] > 0.0)):
    #
    #         kb_fdata[i] = 0.00002 * (((sig_wv_ht_fdata[i] * 100.0) * (friction_fdata[i]) * 100 * 3600) / (
    #                     kinematic_fdata[i] * 36000000.0))
    #         kb_fdata[i] = kb_fdata[i]  # /36.0 conversion from cm/h to 10^-4 m/s (100/3600) = 1/36
    #     else:
    #         kb_fdata[i] = DataLayer.missing_value
    #
    #     # kd component
    #     if ((scskin_fdata[i] != DataLayer.missing_value) and (sigma0_fdata[i] != DataLayer.missing_value) and (
    #             scskin_fdata[i] > 0.0)):
    #
    ...
    #
    #         kd_fdata[i] = (
    #                     ((2100 / (sigma0_fdata[i] * sigma0_fdata[i])) + 0.1) * (pow((scskin_fdata[i] / 600.0), -0.5)))
    #
...
def OceanFluxGHG_kt(kd_fdata, kb_fdata, kb_weighting, kd_weighting):
    ...
    ktotal_fdata = np.full(dataLength, DataLayer.missing_value)

    valid_kd = kd_fdata != DataLayer.missing_value
    valid_kb = kb_fdata != DataLayer.missing_value

    # Both valid
    ktotal_fdata[valid_kd & valid_kb] = (
            kd_weighting * kd_fdata[valid_kd & valid_kb] +
            kb_weighting * kb_fdata[valid_kd & valid_kb]
    )

    # Only kd valid
    ktotal_fdata[valid_kd & ~valid_kb] = kd_fdata[valid_kd & ~valid_kb]

    # Only kb valid
    ktotal_fdata[~valid_kd & valid_kb] = kb_fdata[~valid_kd & valid_kb]
...
class k_Ho2006(KCalculationBase):
    ...
    def __call__(self, data):
        ...
        valid_k = (
                (self.windu10 != DataLayer.missing_value) &
                (self.windu10_moment2 != DataLayer.missing_value) &
                (self.windu10_moment3 != DataLayer.missing_value) &
                (self.scskin != DataLayer.missing_value) &
                (self.scskin > 0.0)
        )
        ...
        #     if ((self.windu10[i] != DataLayer.missing_value) and (
        #             self.windu10_moment2[i] != DataLayer.missing_value) and (
        #             self.windu10_moment3[i] != DataLayer.missing_value) and (
        #             self.scskin[i] != DataLayer.missing_value) and (self.scskin[i] > 0.0)):
...
class k_Nightingale2000(KCalculationBase):
    ...
    def __call__(self, data):
        ...
        self.k[:] = DataLayer.missing_value
        self.k[valid_mask] = (0.222 * self.windu10_moment2[valid_mask] + 0.333 * self.windu10[valid_mask]) * np.sqrt(
            600.0 / self.scskin[valid_mask])

        # for i in arange(len(self.k)):
        #     self.k[i] = DataLayer.missing_value
        #     if ((self.windu10[i] != DataLayer.missing_value) and (
        #             self.windu10_moment2[i] != DataLayer.missing_value) and (
        #             self.scskin[i] != DataLayer.missing_value) and (
        #             self.scskin[i] > 0.0)):  # SOCATv4 - No need for wind moment3
...
class k_Wanninkhof1992(KCalculationBase):
    ...
    def __call__(self, data):
        ...
        self.k[:] = DataLayer.missing_value
        self.k[valid_mask] = 0.31 * self.windu10_moment2[valid_mask] * np.sqrt(660.0 / self.scskin[valid_mask])
        ...
        #             self.windu10_moment2[i] != DataLayer.missing_value) and (
        #             self.scskin[i] != DataLayer.missing_value) and (
        #             self.scskin[i] > 0.0)):  # SOCATv4 - no need for wind moment 3
...
class k_McGillis2001(KCalculationBase):
    ...
    def __call__(self, data):
        ...
        self.k[:] = DataLayer.missing_value
        self.k[valid_mask] = (3.3 + 0.026 * self.windu10_moment3[valid_mask]) * np.sqrt(660.0 / self.scskin[valid_mask])
...
class k_Ho1997(KCalculationBase):
    ...
    def __call__(self, data):
        ...
        valid_mask = (
                ...
                (self.scskin != DataLayer.missing_value) &
                (self.scskin > 0.0) &
                (self.rain != DataLayer.missing_value)
        )
        self.k[:] = DataLayer.missing_value
        self.krain[:] = DataLayer.missing_value
        self.k[valid_mask] = (0.929 + 0.679 * self.rain[valid_mask] - 0.0015 * (self.rain[valid_mask] ** 2)) * np.sqrt(
            600.0 / self.scskin[valid_mask])
        self.krain[valid_mask] = self.k[valid_mask]
        ...
        #             self.windu10_moment2[i] != DataLayer.missing_value) and (
        #             self.windu10_moment3[i] != DataLayer.missing_value) and (
        #             self.scskin[i] != DataLayer.missing_value) and (self.scskin[i] > 0.0) and (
        #             self.rain[i] != DataLayer.missing_value)):
...
class kd_OceanFluxGHG_backscatter(KCalculationBase):
    ...
    def __call__(self, data):
        ...
        try:
            ...
            data[
                "k"].longName = "direct component of gas transfer velocity of carbon dioxide derived from radar backscatter"
        except KeyError as e:
            print("%s: Required data layer for selected k parameterisation was not found." % function)
            print(type(e), e.args)
            return False
...
class k_generic(KCalculationBase):
    def __init__(self, k_generic_a0, k_generic_a1, k_generic_a2, k_generic_a3, k_generic_sc):
        ...
        if self.k_generic_sc != 600 and self.k_generic_sc != 660:
            raise ValueError(
                "%s: Could not initialise k_generic rate parameterisation. Only schmidt numbers (k_generic_sc) of 600 and 660 are allowed, but a schmidt number of %d was provided." % (
                    function, k_generic_sc))
    ...
    def __call__(self, data):
        function = inspect.stack()[0][1] + ", " + inspect.stack()[0][3]
        print(
            "%s Using the general cubic form of k with user specified values (k_generic_a0:%lf k_generic_a1:%lf k_generic_a2:%lf k_generic_a3:%lf k_generic_sc:%lf)" % (
                function, self.k_generic_a0, self.k_generic_a1, self.k_generic_a2, self.k_generic_a3,
                self.k_generic_sc))
        ...
        try:
            ...
            data["k"].longName = "User defined generic formulation of k (a0:%lf a1:%lf a2:%lf a3:%lf sc:%lf)" % (
                self.k_generic_a0, self.k_generic_a1, self.k_generic_a2, self.k_generic_a3, self.k_generic_sc)
        except KeyError as e:
            print("%s: Required data layer for selected k parameterisation was not found." % function)
            print(type(e), e.args)
            return False
        ...
        self.k[:] = DataLayer.missing_value
        self.k[valid_mask] = (self.k_generic_a0
                              + self.k_generic_a1 * self.windu10[valid_mask]
                              + self.k_generic_a2 * self.windu10_moment2[valid_mask]
                              + self.k_generic_a3 * self.windu10_moment3[valid_mask]
                              ) * np.sqrt(self.k_generic_sc / self.scskin[valid_mask])
        ...
        #         self.k[i] = self.k_generic_a0 + (self.k_generic_a1 * self.windu10[i]) + (
        #                 self.k_generic_a2 * self.windu10_moment2[i]) + (self.k_generic_a3 * self.windu10_moment3[i])
...
class kd_OceanFluxGHG_wind(KCalculationBase):
    ...
    def __call__(self, data):
        ...
        try:
            ...
            data[
                "k"].longName = "direct component of gas transfer velocity of carbon dioxide derived from wind speed (Goddijn-Murphy et al., 2012)"
        except KeyError as e:
            print("%s: Required data layer for selected k parameterisation was not found." % function)
            print(type(e), e.args)
            return False
...
class kt_OceanFluxGHG_kd_wind(KCalculationBase):
    ...
    def __call__(self, data):
        ...
        try:
            ...
            data[
                "k"].longName = "total (direct_from_wind_speed plus bubble mediated) component of gas transfer velocity of carbon dioxide"
        except KeyError as e:
            print("%s: Required data layer for selected k parameterisation was not found." % function)
            print(type(e), e.args)
            return False
...
class k_Wanninkhof2014(KCalculationBase):
    ...
    def __call__(self, data):
        ...
        self.k[:] = DataLayer.missing_value
        self.k[valid_mask] = 0.251 * self.windu10_moment2[valid_mask] * np.sqrt(660.0 / self.scskin[valid_mask])
...
class k_Nightingale2000_with_surfactant_suppression(KCalculationBase):
    ...
    def __call__(self, data):
        ...
        valid_mask = (self.SSTSkin != DataLayer.missing_value) & (self.k != DataLayer.missing_value)
        self.k[valid_mask] *= 1.0 - (0.0046 * (self.SSTSkin[valid_mask] - 273.15) ** 2.5673) / 100.0
...
class k_Zappa2007(KCalculationBase):
    ...
    def __call__(self, data):
        ...
        try:
            ...
            data[
                "k"].longName = "Zappa 2007: using the dissipation rate of turbulent kinetic energy. Zappa, Christopher J., Wade R. McGillis, Peter A. Raymond, James B. Edson, Eric J. Hintsa, Hendrik J. Zemmelink, John WH Dacey, and David T. Ho. Environmental turbulent mixing controls on air‐water gas exchange in marine and aquatic systems. Geophysical Research Letters 34, no. 10 (2007). The dissipation rate of turbulent kinetic energy was scaled by calibration factor: " + str(
                self.k_Zappa2007_epsilon_calibration)
        except KeyError as e:
            print("%s: Required data layer for selected k parameterisation was not found." % function)
            print(type(e), e.args)
            return False
        ...
        # determine the k relationship
        valid_mask = (
                (self.tke_dissipation != DataLayer.missing_value) &
                (self.scskin != DataLayer.missing_value) &
                (self.scskin > 0.0)
        )
        self.k[:] = DataLayer.missing_value
        self.k[valid_mask] = (
                self.k_Zappa2007_epsilon_calibration *
                (0.419 * self.scskin[valid_mask] ** -0.5) *
                (self.tke_dissipation[valid_mask] * (
                        0.00000183 * np.exp(-(self.sstskin[valid_mask] - 273.15) / 36.0))) ** 0.25
        )
        self.k[valid_mask] *= 100.0 * 3600.0

        # for i in arange(len(self.k)):
        #     if ((self.tke_dissipation[i] != DataLayer.missing_value) and (
        #             self.scskin[i] != DataLayer.missing_value) and (
        #             self.scskin[i] > 0.0)):  # SOCATv4 - no need for wind moment 3
        #         kinematicViscosity = 0.00000183 * exp(-(self.sstskin[
        #                                                     i] - 273.15) / 36.0)  # Parameterisation from previously used study (GHGOceanFlux, see above k parameterisations)
        #         # kinematicViscosity = self.si_viscosity_to_cSt(kinematicViscosity) #Convert from m^2 s^-1 to cSt (centistokes)
        #
        #         self.k[i] = self.k_Zappa2007_epsilon_calibration * (0.419 * (self.scskin[i] ** -0.5)) * (
        #                 (self.tke_dissipation[i] * kinematicViscosity) ** 0.25)  # new
...
class AddKRainLinearHo1997(KCalculationExtension):
    ...
    def __call__(self, data):
        ...
        self.krain[mask] = (0.929 + (0.679 * self.rain[mask]) - (0.0015 * (self.rain[mask] ** 2.0))
                           ) * ((600.0 / self.scskin[mask]) ** 0.5)

        self.k[mask] = self.k[mask] + self.krain[mask]
        self.krain[~mask] = DataLayer.missing_value

        # for i in arange(len(self.k)):
        #     if ((self.k[i] != DataLayer.missing_value) and (self.scskin[i] != DataLayer.missing_value) and (
        #             self.rain[i] != DataLayer.missing_value) and (self.scskin[i] > 0.0) and (
        #             self.windu10[i] != DataLayer.missing_value) and (
        #             self.windu10_moment2[i] != DataLayer.missing_value) and (
        #             self.windu10_moment3[i] != DataLayer.missing_value)):
...
class AddKRainNonlinearHarrison2012(KCalculationExtension):
    ...
    def __call__(self, data):
        ...
        windstress_fdata = np.full(length, DataLayer.missing_value)
        winddrag_fdata = np.full(length, DataLayer.missing_value)
        ...
        winddrag_fdata[~valid_mask] = DataLayer.missing_value
        winddrag_fdata[valid_mask] = -999.0  # default

        mask_6_to_26 = valid_mask & (self.windu10 >= 6.0) & (self.windu10 < 26.0)
        winddrag_fdata[mask_6_to_26] = 0.60 + 0.070 * self.windu10[mask_6_to_26]

        mask_lt_6 = valid_mask & (self.windu10 < 6.0) & (self.windu10 > 0.0)
        winddrag_fdata[mask_lt_6] = (
                0.29 + (3.1 / self.windu10[mask_lt_6]) + (7.7 / self.windu10_moment2[mask_lt_6])
        )

        # Compute windstress
        windstress_fdata[:] = DataLayer.missing_value
        windstress_fdata[valid_mask] = 0.0

        # Set windstress to -999.0 if winddrag or windu10_moment2 is -999.0
        mask_minus_999 = valid_mask & ((winddrag_fdata == -999.0) | (self.windu10_moment2 == -999.0))
        windstress_fdata[mask_minus_999] = -999.0

        # Else calculate sqrt if both winddrag and windu10_moment2 are nonzero
        mask_calc_stress = valid_mask & (winddrag_fdata != 0.0) & (self.windu10_moment2 != 0.0) & (
            ~mask_minus_999)
        windstress_fdata[mask_calc_stress] = np.sqrt(
            (winddrag_fdata[mask_calc_stress] / 1000.0) * self.windu10_moment2[mask_calc_stress])
        ...
        valid_mask = (
                (self.k != DataLayer.missing_value) &
                (self.scskin != DataLayer.missing_value) & (self.scskin > 0.0) &
                (self.rain != DataLayer.missing_value) &
                (self.windu10 != DataLayer.missing_value) &
                (self.windu10_moment2 != DataLayer.missing_value) &
                (self.windu10_moment3 != DataLayer.missing_value) &
                (self.pressure != DataLayer.missing_value) &
                (self.sstskin != DataLayer.missing_value) &
                (windstress_fdata != DataLayer.missing_value)
        )

        if valid_mask.any():
            rho_a = (self.pressure[valid_mask] * 100.0) / (R * self.sstskin[valid_mask])
            beta_r = (0.0112 * self.rain[valid_mask]) / (rho_a * windstress_fdata[valid_mask] ** 3)
            self.krain[valid_mask] = (1 - np.exp(-alpha_r * beta_r)) * (
                    63.02 * (0.0112 * self.rain[valid_mask]) ** 0.6242)
            self.krain[valid_mask] *= np.sqrt(600.0 / self.scskin[valid_mask])
            self.k[valid_mask] += self.krain[valid_mask]
        ...
        #             self.windu10_moment2[i] != DataLayer.missing_value) and (
        #             self.windu10_moment3[i] != DataLayer.missing_value) and (
        #             self.pressure[i] != DataLayer.missing_value) and (self.sstskin[i] != DataLayer.missing_value) and (
        #             windstress_fdata[i] != DataLayer.missing_value)):
        #         # guts in here
...
# Nightingale, P. D., et al. 2000. In situ evaluation of air-sea gas exchange parameterizations using novel conserva-tive  and  volatile  tracers.  Global  Biogeochem.  Cycles14:373-387 [doi:10. 1029/ 1999GB900091].
# + modification for bubbles as being determined within the ESA Ocean Carbon 4 Climate project
class k_Nightingale2000_with_bubbles(KCalculationBase):
    def __init__(self,f):
        self.name = self.__class__.__name__
        self.f = f

    def input_names(self):
        return ["windu10", "windu10_moment2", "scskin", "scfnd","solubility_fnd"]

    def output_names(self):
        return ["k","kd","kb","kd","Ostwald","whitecap","kd_dms","X"]

    def __call__(self, data):
        function = "(rate_parameterisation.py: k_Nightingale2000_with_bubbles.__call__)"
        print("%s Using the Nightingale et al., 2000 (N00) with bubbles k parameterisation" % (function))
        try:
            # for ease of access, simply assign attributes to each input/output.
            for name in self.input_names() + self.output_names():
                setattr(self, name, data[name].fdata)
        ...
        valid_mask = (
                ...
                (self.windu10_moment2 != DataLayer.missing_value) &
                (self.scskin != DataLayer.missing_value) &
                (self.scfnd != DataLayer.missing_value) &
                (self.solubility_fnd != DataLayer.missing_value))
        # Calcualting the standard Nightingale et al. K as k_total (kt)
        self.k[:] = DataLayer.missing_value
        self.k[valid_mask] = (0.222 * self.windu10_moment2[valid_mask] + 0.333 * self.windu10[valid_mask]) * np.sqrt(
            600.0 / self.scskin[valid_mask])

        #Calcualte Whitecapping following the MAP defintion (Callaghan et al., 2008; https://doi.org/10.1029/2008GL036165)
        self.whitecap[:] = DataLayer.missing_value
        white = valid_mask & (self.windu10 <= 10.18)
        self.whitecap[white] = 0.0000318*(self.windu10[white]-3.7)**3
        white = valid_mask & (self.windu10 > 10.18)
        self.whitecap[white] = 0.00000482*(self.windu10[white]+1.98)**3

        self.Ostwald[:] = DataLayer.missing_value
        self.Ostwald[valid_mask] = ((self.solubility_fnd[valid_mask]/103.25)*2478.96)

        self.X[:] = DataLayer.missing_value
        self.X[valid_mask] = (self.scfnd[valid_mask]**0.5) / (14 * self.Ostwald[valid_mask])

        self.kb[:] = DataLayer.missing_value
        self.kb[valid_mask] = 2450 * self.whitecap[valid_mask] *(( (1+(self.X[valid_mask]**(1/self.f)))**(-self.f)) / self.Ostwald[valid_mask])

        #At very low whitecapping the kb equation can become negative (~ -1cmhr) so we restrict these to 0.
        self.kb[(self.kb<0) &(self.kb>-10)] = 0.0 # Specify a maximum -10 cmhr so we don't change the fill value (default is -999)

        self.kd[:] = DataLayer.missing_value
        self.kd[valid_mask] = self.k[valid_mask] - self.kb[valid_mask]

        self.kd_dms[:] = DataLayer.missing_value
        self.kd_dms[valid_mask] = (2.6 * self.windu10[valid_mask] - 5.7) * np.sqrt(660.0 / self.scskin[valid_mask])

        return True

class k_generic_with_bubbles(KCalculationBase):
    def __init__(self, k_generic_a0, k_generic_a1, k_generic_a2, k_generic_a3, k_generic_sc,f):
        function = inspect.stack()[0][1] + ", " + inspect.stack()[0][3]
        self.name = self.__class__.__name__
        self.k_generic_a0 = k_generic_a0
        self.k_generic_a1 = k_generic_a1
    ...
    def input_names(self):
        return ["windu10", "windu10_moment2", "windu10_moment3", "scskin","scfnd","solubility_fnd"]
    ...
    def __call__(self, data):
        function = inspect.stack()[0][1] + ", " + inspect.stack()[0][3]
        print(
            "%s Using the general cubic form of k with user specified values with bubbles (k_generic_a0:%lf k_generic_a1:%lf k_generic_a2:%lf k_generic_a3:%lf k_generic_sc:%lf)" % (
                function, self.k_generic_a0, self.k_generic_a1, self.k_generic_a2, self.k_generic_a3,
                self.k_generic_sc))
        ...
        try:
            ...
            for name in self.input_names() + self.output_names():
                setattr(self, name, data[name].fdata)
            data["k"].standardName = "generic_gas_transfer_velocity_formulation with bubbles"
            data["k"].longName = "User defined generic formulation of k with bubbles (a0:%lf a1:%lf a2:%lf a3:%lf sc:%lf)" % (
                self.k_generic_a0, self.k_generic_a1, self.k_generic_a2, self.k_generic_a3, self.k_generic_sc)
        except KeyError as e:
            print("%s: Required data layer for selected k parameterisation was not found." % function)
            print(type(e), e.args)
...
class k_generic_with_Dongbubbles(KCalculationBase):
    def __init__(self, k_generic_a0, k_generic_a1, k_generic_a2, k_generic_a3, k_generic_sc):
        function = inspect.stack()[0][1] + ", " + inspect.stack()[0][3]
        self.name = self.__class__.__name__
        self.k_generic_a0 = k_generic_a0
    ...
    def __call__(self, data):
        ...
        self.kd[:] = DataLayer.missing_value
        # self.kd[valid_mask] = (2.6 * self.windu10[valid_mask] - 5.7) * np.sqrt(660.0 / self.scskin[valid_mask]) # Goddijn Murphy et al. 2016
        self.kd[valid_mask] = (0.7432*self.windu10[valid_mask]**1.33) * np.sqrt(660.0 / self.scskin[valid_mask]) # Blomquist et al. 2017 https://doi.org/10.1002/2017JC013181

        self.kb[:] = DataLayer.missing_value
        self.kb[valid_mask] = self.k[valid_mask] - self.kd[valid_mask]
        self.kb[valid_mask & (self.windu10<5)] = 0
        return True
