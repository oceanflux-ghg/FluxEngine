...
def write_netcdf(fluxEngineObject, verbose=False):
    ...
    if outputChunk == 0:  # Create a new file and write output to it.
        ...
        if (runParams.temporal_resolution != "monthly") and (
                runParams.temporal_resolution != None):  # Fill in all the time points. Can't really do this with monthly temporal resolution.
        ...
        if len(latitudeData.shape) < 2:  # IGA - If the initial latitude data was a vector, write data as vectors
            lats2 = ncfile.createVariable('latitude', dtype('float64').char, ('latitude'), zlib=True, complevel=6)
            lons2 = ncfile.createVariable('longitude', dtype('float64').char, ('longitude'), zlib=True, complevel=6)
            # Assign units attributes to coordinate var data. This attaches a
            # text attribute to each of the coordinate variables, containing the
            # units.
            ...
            lats = ncfile.createVariable('latitude', dtype('float64').char, dims, zlib=True, complevel=6);
            lons = ncfile.createVariable('longitude', dtype('float64').char, dims, zlib=True, complevel=6);
            # Assign units attributes to coordinate var data. This attaches a
            # text attribute to each of the coordinate variables, containing the
            # units.
        ...
        setattr(ncfile, 'Institution',
                'Originally developed by the partners of the ESA OceanFlux GHG and OceanFlux GHG Evolution projects. Now continued by the CarbonLab team at the University of Exeter.')
        setattr(ncfile, 'Contact', 'email: j.d.shutler@exeter.ac.uk')
        setattr(ncfile, 'FluxEngine_version', fe_v)
...
def calculate_solubility_distilled(salinity, rain_wet_deposition_switch, sstskin, deltaT, nx, ny,
                                   schmidtParameterisation, gasStr):
    # First create a 0 salinity dataset
    salDistil = np.full(len(salinity), missing_value)  # array([missing_value] * len(salinity))
    salDistil[salinity != missing_value] = 0.0
...
def calculate_whitecapping(windu10, whitecap):
    if (not isinstance(windu10, DataLayer)) or (not isinstance(whitecap, DataLayer)):
        raise ValueError(
            "ofluxghg_flux_calc.calculate_whitecapping: Invalid arguments. Arguments must be DataLayer type.");
    if windu10.nx != whitecap.nx or windu10.ny != whitecap.ny:
        raise ValueError(
            "ofluxghg_flux_calc.calculate_whitecapping: Invalid arguments. windu10 and whitecap dimensions do not match (%d, %d vs %d, %d)." % (
                windu10.nx, windu10.ny, whitecap.nx, whitecap.ny));

    mask = windu10.fdata != missing_value
    whitecap.fdata[mask] = 0.00159 * (windu10.fdata[mask] ** 2.7)
    whitecap.fdata[~mask] = missing_value
    # for i in arange(windu10.nx * windu10.ny):
    #     if (windu10.fdata[i] != missing_value):
    #         whitecap.fdata[i] = 0.00159 * np.power(windu10.fdata[i], 2.7)
    #     else:
...
def add_noise(data, rmse_value, mean_value, no_elements, clipNegative=False):
    ...
    if clipNegative:
        ...
        if rmse_value / abs(meanVal) > 0.2:
            print(
                "WARNING: Adding noise to input datalayer but RMSE value is > 20% of the mean value for this input. This may result in negative values which will be clipped at 0, and therefore indirectly add bias to the input data.");
        print("INFO: A total of %d grid cells were clipped resulting in an approximate bias of %f." % (
            numClipped, totalClipped / no_elements));
...
def add_noise_and_bias_wind(winduData, moment2, moment3, rmseValue, meanValue, biasValue, numElements):
    ...
    print(
        "INFO: Adding noise to wind overwrites windu10_moment2 and windu10_moment3 with (windu10+epsilon)^2 and (windu10+epsilon)^3.");
    ...
    if rmseValue / meanVal > 0.2:
        print(
            "WARNING: Adding noise to input datalayer but RMSE value is > 20% of the mean value for this input. This may result in negative values which will be clipped at 0, and therefore indirectly add bias to the input data.");
        print("WARNING: A total of %d grid cells were clipped resulting in an approximate additional bias of %f." % (
            numClipped, totalClipped / numElements));
...
def add_bias_k_biology_wind(data, bias_k_value, biology_data, biology_value, wind_data, wind_value, no_elements,
                            percent_switch):
    # adding bias offset to data array based on biology and wind conditions
    mask = (
            (data != missing_value) &
            (biology_data != missing_value) &
            (wind_data != missing_value) &
            (biology_data >= biology_value) &
            (wind_data <= wind_value)
    )

    data[mask] += (-data[mask] * (bias_k_value / 100.0)) if percent_switch == 1 else bias_k_value
...
def median_filter2D(datain, nx, ny):
    ...
    vector = np.full(size ** 2, 0.0)  # array([0.0] * size * size)
...
def schmidt_Wanninkhof1992(sstC_fdata, nx, ny, gas):
    ...
    sc_fdata = np.full(nx * ny, missing_value)
    if 'o2' in gas.lower():
        mask = (sstC_fdata != missing_value)
        sc_fdata[mask] = (
                1953.4
                - 128.0 * sstC_fdata[mask]
                + 3.9918 * sstC_fdata[mask] ** 2
                - 0.050091 * sstC_fdata[mask] ** 3
        )
        sc_fdata[~mask] = missing_value

        # for i in arange(nx * ny):
        #     if (sstC_fdata[i] != missing_value):
        #         sc_fdata[i] = 1953.4 - (128.0 * sstC_fdata[i]) + (3.9918 * (sstC_fdata[i] * sstC_fdata[i])) - (
        #                     0.050091 * (sstC_fdata[i] * sstC_fdata[i] * sstC_fdata[i]))  # IGA-O2
        #     else:
    ...
    if 'n2o' in gas.lower():
        ...
        sc_fdata[mask] = (
                2301.1
                - 151.1 * sstC_fdata[mask]
                + 4.7364 * sstC_fdata[mask] ** 2
                - 0.059431 * sstC_fdata[mask] ** 3
        )
        sc_fdata[~mask] = missing_value

        # for i in arange(nx * ny):
        #     if (sstC_fdata[i] != missing_value):
        #         sc_fdata[i] = 2301.1 - (151.1 * sstC_fdata[i]) + (4.7364 * (sstC_fdata[i] * sstC_fdata[i])) - (
        #                     0.059431 * (sstC_fdata[i] * sstC_fdata[i] * sstC_fdata[i]))  # IGA-N2O
        #     else:
        #         # assigning invalid values
        #         sc_fdata[i] = missing_value
    ...
    if 'ch4' in gas.lower():
        ...
        sc_fdata[mask] = (
                2039.2
                - 120.31 * sstC_fdata[mask]
                + 3.4209 * sstC_fdata[mask] ** 2
                - 0.040437 * sstC_fdata[mask] ** 3
        )
        sc_fdata[~mask] = missing_value

        # for i in arange(nx * ny):
        #     if (sstC_fdata[i] != missing_value):
        #         sc_fdata[i] = 2039.2 - (120.31 * sstC_fdata[i]) + (3.4209 * (sstC_fdata[i] * sstC_fdata[i])) - (
        #                     0.040437 * (sstC_fdata[i] * sstC_fdata[i] * sstC_fdata[i]))  # IGA-CH4
        #     else:
        #         # assigning invalid values
        #         sc_fdata[i] = missing_value
    if 'co2' in gas.lower():
        ...
        sc_fdata[mask] = (
                2073.1
                - 125.62 * sstC_fdata[mask]
                + 3.6276 * sstC_fdata[mask] ** 2
                - 0.043219 * sstC_fdata[mask] ** 3
        )
        sc_fdata[~mask] = missing_value
        ...
        #         sc_fdata[i] = 2073.1 - (125.62 * sstC_fdata[i]) + (3.6276 * (sstC_fdata[i] * sstC_fdata[i])) - (
        #                     0.043219 * (sstC_fdata[i] * sstC_fdata[i] * sstC_fdata[i]))  # IGA-CO2
        #     else:
        #         # assigning invalid values
        #         sc_fdata[i] = missing_value
    return sc_fdata
...
def schmidt_Wanninkhof2014(sstC_fdata, nx, ny, gas):
    ...
    if 'o2' in gas.lower():
        ...
        sc_fdata[mask] = (
                1920.4
                - 135.6 * sstC_fdata[mask]
                + 5.2122 * sstC_fdata[mask] ** 2
                + 0.10939 * sstC_fdata[mask] ** 3
                + 0.00093777 * sstC_fdata[mask] ** 4
        )
        sc_fdata[~mask] = missing_value

        # for i in arange(nx * ny):
        #     if (sstC_fdata[i] != missing_value):
        #         sc_fdata[i] = 1920.4 + (-135.6 * sstC_fdata[i]) + (5.2122 * sstC_fdata[i] ** 2) + (
        #                     0.10939 * sstC_fdata[i] ** 3) + (0.00093777 * sstC_fdata[i] ** 4);
        #     else:
        #         # assigning invalid values
        #         sc_fdata[i] = missing_value
    ...
    if 'n2o' in gas.lower():
        ...
        sc_fdata[mask] = (
                2356.2
                - 166.38 * sstC_fdata[mask]
                + 6.3952 * sstC_fdata[mask] ** 2
                - 0.13422 * sstC_fdata[mask] ** 3
                + 0.0011506 * sstC_fdata[mask] ** 4
        )
        sc_fdata[~mask] = missing_value

        # for i in arange(nx * ny):
        #     if (sstC_fdata[i] != missing_value):
        #         sc_fdata[i] = 2356.2 + (-166.38 * sstC_fdata[i]) + (6.3952 * sstC_fdata[i] ** 2) + (
        #                     -0.13422 * sstC_fdata[i] ** 3) + (0.0011506 * sstC_fdata[i] ** 4);
        #     else:
        #         # assigning invalid values
        #         sc_fdata[i] = missing_value
    ...
    if 'ch4' in gas.lower():
        ...
        sc_fdata[mask] = (
                2101.2
                - 131.54 * sstC_fdata[mask]
                + 4.4931 * sstC_fdata[mask] ** 2
                - 0.08676 * sstC_fdata[mask] ** 3
                + 0.00070663 * sstC_fdata[mask] ** 4
        )
        sc_fdata[~mask] = missing_value

        # for i in arange(nx * ny):
        #     if (sstC_fdata[i] != missing_value):
        #         sc_fdata[i] = 2101.2 + (-131.54 * sstC_fdata[i]) + (4.4931 * sstC_fdata[i] ** 2) + (
        #                     -0.08676 * sstC_fdata[i] ** 3) + (0.00070663 * sstC_fdata[i] ** 4);
        #     else:
        #         # assigning invalid values
        #         sc_fdata[i] = missing_value
    if 'co2' in gas.lower():
        ...
        sc_fdata[mask] = (
                2116.8
                - 136.25 * sstC_fdata[mask]
                + 4.7353 * sstC_fdata[mask] ** 2
                - 0.092307 * sstC_fdata[mask] ** 3
                + 0.0007555 * sstC_fdata[mask] ** 4
        )
        sc_fdata[~mask] = missing_value
        ...
        #     if (sstC_fdata[i] != missing_value):
        #         # relationship is only valid for temperatures <=30.0 oC
        #         sc_fdata[i] = 2116.8 + (-136.25 * sstC_fdata[i]) + (4.7353 * sstC_fdata[i] ** 2) + (
        #                     -0.092307 * sstC_fdata[i] ** 3) + (0.0007555 * sstC_fdata[i] ** 4);
        #     else:
        #         # assigning invalid values
        #         sc_fdata[i] = missing_value
    return sc_fdata
...
def solubility_Wanninkhof1992(sstK, sal, deltaT, nx, ny, flux_calc, gas):
    ...
    sol = np.full(nx * ny, missing_value)
    if gas == 'co2':
        mask = (sstK != missing_value) & (sal != missing_value) & (sstK > 0.0)
        sol[mask] = (
                -60.2409
                + 93.4517 * (100.0 / sstK[mask])
                + 23.3585 * np.log(sstK[mask] / 100.0)
                + sal[mask] * (
                        0.023517
                        - 0.023656 * (sstK[mask] / 100.0)
                        + 0.0047036 * (sstK[mask] / 100.0) ** 2
                )
        )
        sol[mask] = np.exp(sol[mask])

        if flux_calc != 2:
            deltaT[mask] = 0.0
            sol[mask] = sol[mask] * (1 - (0.015 * deltaT[mask]))

        sol[~mask] = missing_value

        # for i in arange(nx * ny):
        #     if ((sstK[i] != missing_value) and (sal[i] != missing_value) and (sstK[i] > 0.0)):
        #         sol[i] = -60.2409 + (93.4517 * (100.0 / sstK[i])) + (23.3585 * (np.log(sstK[i] / 100.0))) + (sal[i] * (
        #                     0.023517 + ((-0.023656) * (sstK[i] / 100.0)) + (
        #                         0.0047036 * ((sstK[i] / 100.0) * (sstK[i] / 100.0)))));
        #         sol[i] = np.exp(sol[i])
        ...
        sol[mask] = (
                -58.3877
                + 85.8079 * (100.0 / sstK[mask])
                + 23.8439 * np.log(sstK[mask] / 100.0)
                + sal[mask] * (
                        -0.034892
                        + 0.015568 * (sstK[mask] / 100.0)
                        - 0.0019387 * (sstK[mask] / 100.0) ** 2
                )
        )
        sol[mask] = np.exp(sol[mask])
        ...
        # for i in arange(nx * ny):
        #     if ((sstK[i] != missing_value) and (sal[i] != missing_value) and (sstK[i] > 0.0)):
        #         sol[i] = -58.3877 + (85.8079 * (100.0 / sstK[i])) + (23.8439 * (np.log(sstK[i] / 100.0))) + (sal[i] * (
        #                     -0.034892 + ((0.015568) * (sstK[i] / 100.0)) + (
        #                         -0.0019387 * ((sstK[i] / 100.0) * (sstK[i] / 100.0)))));
        #         sol[i] = np.exp(sol[i])
        #         # runParams.flux_calc is a switch to remove Delta_T-Sb component - ie selects use of RAPID or EQUILIBRIUM flux models from Woolf et al., 2016
        #         if flux_calc != 2:
        #             deltaT[i] = 0.0
        ...
        sol[mask] = (
                -64.8539
                + 100.2520 * (100.0 / sstK[mask])
                + 25.2049 * np.log(sstK[mask] / 100.0)
                + sal[mask] * (
                        -0.062544
                        + 0.035337 * (sstK[mask] / 100.0)
                        - 0.0054699 * (sstK[mask] / 100.0) ** 2
                )
        )
        sol[mask] = np.exp(sol[mask])
        ...
        # for i in arange(nx * ny):
        #     if ((sstK[i] != missing_value) and (sal[i] != missing_value) and (sstK[i] > 0.0)):
        #         sol[i] = -64.8539 + (100.2520 * (100.0 / sstK[i])) + (25.2049 * (np.log(sstK[i] / 100.0))) + (sal[i] * (
        #                     -0.062544 + ((0.035337) * (sstK[i] / 100.0)) + (
        #                         -0.0054699 * ((sstK[i] / 100.0) * (sstK[i] / 100.0)))));
        #         sol[i] = np.exp(sol[i])
        #         # runParams.flux_calc is a switch to remove Delta_T-Sb component - ie selects use of RAPID or EQUILIBRIUM flux models from Woolf et al., 2016
        #         if flux_calc != 2:
        #             deltaT[i] = 0.0
        ...
        sol[mask] = (
                -68.8862
                + 101.4956 * (100.0 / sstK[mask])
                + 28.7314 * np.log(sstK[mask] / 100.0)
                + sal[mask] * (
                        -0.076146
                        + 0.043970 * (sstK[mask] / 100.0)
                        - 0.0068672 * (sstK[mask] / 100.0) ** 2
                )
        )
        sol[mask] = np.exp(sol[mask])
        ...
        # for i in arange(nx * ny):
        #     if ((sstK[i] != missing_value) and (sal[i] != missing_value) and (sstK[i] > 0.0)):
        #         sol[i] = -68.8862 + (101.4956 * (100.0 / sstK[i])) + (28.7314 * (np.log(sstK[i] / 100.0))) + (sal[i] * (
        #                     -0.076146 + ((0.043970) * (sstK[i] / 100.0)) + (
        #                         -0.0068672 * ((sstK[i] / 100.0) * (sstK[i] / 100.0)))));
        #         sol[i] = np.exp(sol[i])
        #         # runParams.flux_calc is a switch to remove Delta_T-Sb component - ie selects use of RAPID or EQUILIBRIUM flux models from Woolf et al., 2016
        #         if flux_calc != 2:
        #             deltaT[i] = 0.0
...
def solubility_Wanninkhof2014(sstK, sal, deltaT, nx, ny, flux_calc, gas):
    ...
    if gas == 'co2':
        ...
        sol[mask] = (
                -58.0931
                + 90.5069 * (100.0 / sstK[mask])
                + 22.2940 * np.log(sstK[mask] / 100.0)
                + sal[mask] * (
                        0.027766
                        - 0.025888 * (sstK[mask] / 100.0)
                        + 0.0050578 * (sstK[mask] / 100.0) ** 2
                )
        )
        sol[mask] = np.exp(sol[mask])
        ...
        # for i in arange(nx * ny):
        #     if ((sstK[i] != missing_value) and (sal[i] != missing_value) and (sstK[i] > 0.0)):
        #         sol[i] = -58.0931 + (90.5069 * (100.0 / sstK[i])) + (22.2940 * (np.log(sstK[i] / 100.0))) + (sal[i] * (
        #                     0.027766 + ((-0.025888) * (sstK[i] / 100.0)) + (
        #                         0.0050578 * ((sstK[i] / 100.0) * (sstK[i] / 100.0)))));
        #         sol[i] = np.exp(sol[i])
        #         # runParams.flux_calc is a switch to remove Delta_T-Sb component - ie selects use of RAPID or EQUILIBRIUM flux models from Woolf et al., 2016
        #         if flux_calc != 2:
        #             deltaT[i] = 0.0
        ...
        sol[mask] = (
                -62.7062
                + 97.3066 * (100.0 / sstK[mask])
                + 24.1406 * np.log(sstK[mask] / 100.0)
                + sal[mask] * (
                        -0.058420
                        + 0.033193 * (sstK[mask] / 100.0)
                        - 0.0051313 * (sstK[mask] / 100.0) ** 2
                )
        )
        sol[mask] = np.exp(sol[mask])
        ...
        # for i in arange(nx * ny):
        #     if ((sstK[i] != missing_value) and (sal[i] != missing_value) and (sstK[i] > 0.0)):
        #         sol[i] = -62.7062 + (97.3066 * (100.0 / sstK[i])) + (24.1406 * (np.log(sstK[i] / 100.0))) + (sal[i] * (
        #                     -0.058420 + ((0.033193) * (sstK[i] / 100.0)) + (
        #                         -0.0051313 * ((sstK[i] / 100.0) * (sstK[i] / 100.0)))));
        #         sol[i] = np.exp(sol[i])
        #         # runParams.flux_calc is a switch to remove Delta_T-Sb component - ie selects use of RAPID or EQUILIBRIUM flux models from Woolf et al., 2016
        #         if flux_calc != 2:
        #             deltaT[i] = 0.0
...
def calculate_concw(concFactor, foundationSolubility, fCO2water, concw):
    mask = (foundationSolubility != missing_value) & (fCO2water != missing_value)
    concw[mask] = foundationSolubility[mask] * concFactor * fCO2water[mask]
    concw[~mask] = missing_value
...
def calculate_conca(concFactor, skinSolubility, fCO2air, conca):
    mask = (skinSolubility != missing_value) & (fCO2air != missing_value)
    conca[mask] = skinSolubility[mask] * concFactor * fCO2air[mask]
    conca[~mask] = missing_value
...
def copy_missing_values(master, derived, missingValue=DataLayer.missing_value):
    if master.nx != derived.nx and master.ny != derived.ny:
        raise ValueError(
            "copy_missing_values: master and derived DataLayers do not have the same dimensions: (%d, %d) versus (%d, %d)." % (
                master.nx, master.ny, derived.nx, derived.ny));
    mask = (master.fdata == missingValue)
    derived.fdata[mask] = missing_value
...
def check_output_dataset(datalayer, failed_quality_fdata):
    ...
    valid_mask = datalayer.fdata != DataLayer.missing_value

    if datalayer.maxBound is not None:
        high_mask = valid_mask & (datalayer.fdata > datalayer.maxBound)
        first_fail_high = (failed_quality_fdata == DataLayer.missing_value) & high_mask
        failed_quality_fdata[first_fail_high] = 1
        failed_quality_fdata[high_mask & ~first_fail_high] += 1

    if datalayer.minBound is not None:
        low_mask = valid_mask & (datalayer.fdata < datalayer.minBound)
        first_fail_low = (failed_quality_fdata == DataLayer.missing_value) & low_mask
        failed_quality_fdata[first_fail_low] = 1
        failed_quality_fdata[low_mask & ~first_fail_low] += 1
...
def average_pixels(datalayer, nx, ny, missing_value):
    ...
    if len(data.shape) == 2:
        ...
        raise ValueError(
            "%s: Unexpected number of dimensions (%d) when trying to resize data layer." % (function, len(data.shape)));

    if datany % ny != 0 and datanx % nx != 0:
        print(datanx, datany)
        raise ValueError(
            "%s: Cannot rescale data layer because global data layer dimensions are not a whole multiple of the data layer's dimensions." % function);
    n = (datanx / nx);
    n2 = (datany / ny);
    if n != n2:
        raise ValueError("%s: Scaling data layers by irregular scaling factors is not supported (e.g. %d != %d)." % (
            function, n, n2));
...
def check_dimensions(dataLayer, ref_nx, ref_ny, DEBUG=False):
    ...
    if dataLayer.data.shape[1] == ref_nx and dataLayer.data.shape[0] == ref_ny:
        if DEBUG:
            print("\n%s Input data (%s) have identical dimensions to reference values (%s, %s) " % (
                function, dataLayer.name, dataLayer.nx, dataLayer.ny))
            return True;
    else:
        print(
            "\n%s Input data ('%s') dimensions are non-identical to reference (new: %s, %s is not equal to: %s, %s)." % (
                function, dataLayer.name, dataLayer.nx, dataLayer.ny, ref_nx, ref_ny))
        return False;
...
class FluxEngine:
    def __init__(self, parameterDict):
        ...
        self.defaultSettings = Settings(
            path.join(path.dirname(__file__), "settings.xml"));  # Load default settings metadata for datalayers.
        self.data = {};
    ...
    def add_data_layer(self, name, infile, prod, stddevProd=None, countProd=None, transposeData=False,
                       preprocessing=None):
        ...
        if stddevProd != None:
            ...
            try:
                ...
                copy_missing_values(self.data[name], self.data[
                    name + "_stddev"]);  # Any missing values in 'name' should be copied to the stddev dataset for consistency.
            # except TypeError:
            except KeyError as e:
        ...
        if countProd != None:
            ...
            try:
                ...
                copy_missing_values(self.data[name], self.data[
                    name + "_count"]);  # Any missing values in 'name' should be copied to the count dataset for consistency.
            except KeyError:
            ...
            if attribute != name:
                if name + "_" + attribute in vars(self.runParams):
                    if attribute in ["temporalChunking", "temporalSkipInterval"]:  # Integers
                        setattr(metaData, attribute, int(getattr(self.runParams,
                                                                 name + "_" + attribute)));  # TODO: should be handled in setup_tools::create_run_parameters really.
                    else:  # floats
    ...
    def _load_lon_lat_time(self):
        ...
        try:
            ...
            print(
                "Couldn't find file path for axes_data_layer (%s). Check that this is correctly defined in the config file (it must match the name of another datalayer)." % self.runParams.axes_data_layer);
            print(e.args);
        ...
        try:
            ...
            print("\n%s: axes_data_layer (%s) inputfile %s does not exist" % (
                function, self.runParams.axes_data_layer, axesDatalayerInfile));
            print(type(e), e.args);
        ...
        try:
            ...
            raise ValueError(
                "%s: Couldn't find longitude (%s) and/or latitude (%s) variables in %s. Have you set longitude_prod and latitude_prod correctly in your configuration file?" % (
                    function, self.runParams.longitude_prod, self.runParams.latitude_prod, axesDatalayerInfile));

        # Determine if already a grid, if not calculate lon and lat grids.
        if len(self.latitude_data.shape) == 1:  # not already a grid
            self.latitude_grid = meshgrid(ones((len(self.longitude_data))), self.latitude_data)[
                1]  # IGA - gridding vectored geo data so that they can be treated the sam way as non-regular grids
        ...
        try:
            curDatetime = datetime(self.runParams.year, self.runParams.month, self.runParams.day, self.runParams.hour,
                                   self.runParams.minute, self.runParams.second);
            self.time_data = (curDatetime - datetime(1970, 1, 1)).total_seconds();
            # self.time_data = dataset.variables[self.runParams.time_prod][:];
        except KeyError:
            raise ValueError(
                "%s: Couldn't find time (%s%) variables in %s. Have you set time_prod correctly in your configuration file?" % (
                    function, self.runParams.time_prod, self.runParams.sstskin_infile));

        # set dimensions
        self.ny, self.nx = self.latitude_grid.shape;
        print("%s: Grid dimensions set to: (%d, %d)" % (function, self.ny, self.nx));
        # print('Dataset_closed')
        dataset.close();  # DJF - 14/11/2024 - This is needed otherwise the input netCDF remains open. This causes issue for example, if the FluxEngine is run, and then the input file is edited after (i.e HDF error because the file is already open)
    ...
    def _run_fluxengine(self, runParams):
        ...
        if runParams.TAKAHASHI_DRIVER == True:
            ...
            mask = self.data["windu10"].fdata != missing_value
            self.data["windu10_moment2"].fdata[mask] = self.data["windu10"].fdata[mask] ** 2
            self.data["windu10_moment3"].fdata[mask] = self.data["windu10"].fdata[mask] ** 3

            # for i in arange(self.nx * self.ny):
            #     if self.data["windu10"].fdata[i] != missing_value:
            #         self.data["windu10_moment2"].fdata[i] = self.data["windu10"].fdata[i] * self.data["windu10"].fdata[
            #             i]
            #         self.data["windu10_moment3"].fdata[i] = self.data["windu10"].fdata[i] * self.data["windu10"].fdata[
        ...
        self.data["salinity_skin"].fdata = self.data["salinity"].fdata + runParams.saline_skin_value
        if (runParams.saline_skin_value != 0.0):
            print("%s Using the saline skin model (%lf psu added to skin salinities)" % (
        ...
        if (runParams.random_noise_windu10_switch == 1):
            ...
            add_noise_and_bias_wind(self.data["windu10"].fdata, self.data["windu10_moment2"].fdata,
                                    self.data["windu10_moment3"].fdata, runParams.windu10_noise, 0.0,
                                    runParams.windu10_bias, nx * ny);
            print(
                "%s Adding random noise to windu10_mean, windu10_moment2 and windu10_moment3 (mean 0.0, stddev 0.44 ms^-1 - assuming using ESA GlobWave data)" % (
                    function))

        if (runParams.random_noise_sstskin_switch == 1):
            add_noise(self.data["sstskin"].fdata, runParams.sstskin_noise, 0.0, nx * ny)
            print("%s Adding random noise to sstskin (mean 0.0, stddev 0.14 ^oC - assuming using ESA CCI ARC data)" % (
                function))

        if (runParams.random_noise_sstfnd_switch == 1):
            add_noise(self.data["sstfnd"].fdata, runParams.sstfnd_noise, 0.0, nx * ny)
            print(
                "%s Adding random noise to sstfnd (mean 0.0, stddev 0.6 ^oC - assuming using OSTIA data)" % (function))

        if (runParams.random_noise_pco2_switch == 1):
            print("/n%s Shape of pco2 data", self.data["pgas_sw"].fdata.shape)
            add_noise(self.data["pgas_sw"].fdata, runParams.pco2_noise, 0.0, nx * ny,
                      clipNegative=True)  # SOCAT uncertainty
            print(
                "%s Adding random noise to pco2/fco2 data (mean 0.0, stddev 6 uatm - Using Candyfloss data, value provided by J Shutler)" % (
                    function))
        ...
        # sstskin = sstfnd
        if runParams.sst_gradients_switch == 0 and (self._input_data_provided("sstskin") == False) and (
                self._input_data_provided("sstfnd") == True):
            print(
                "%s SST gradient handling is off, using SSTfnd data selection in configuration file for all components of the flux calculation (this will ignore any SSTskin data in configuration file)." % (
                    function))
            ...
            if "sstskin" not in self.data:  # Must add the sstskin layer first!
                ...
                if "sstfnd_count" in self.data:
                    self.add_empty_data_layer("sstskin_count");
            mask = self.data["sstfnd"].fdata != missing_value
            self.data["sstskin"].fdata[mask] = self.data["sstfnd"].fdata[mask]
            self.data["sstskin_stddev"].fdata[mask] = self.data["sstfnd_stddev"].fdata[mask]
            self.data["sstskin_count"].fdata[mask] = self.data["sstfnd_count"].fdata[mask]
            self.data["sstskin"].fdata[~mask] = missing_value
        ...
        # must estimate sstskin (= sstfnd - runParams.cool_skin_difference)
        elif runParams.sst_gradients_switch == 1 and (self._input_data_provided("sstskin") == False) and (
                self._input_data_provided("sstfnd") == True):
            print(
                "%s Using SSTfnd data selection with correction for skin temperature (SSTskin = SSTfnd - %f)(ignoring SSTskin data in configuration file)." % (
                    function, runParams.cool_skin_difference));
            ...
            if "sstskin" not in self.data:  # Must add the sstskin layer first!
                ...
                if "sstfnd_count" in self.data:
                    self.add_empty_data_layer("sstskin_count");
            mask = self.data["sstfnd"].fdata != missing_value
            self.data["sstskin"].fdata[mask] = self.data["sstfnd"].fdata[mask] - runParams.cool_skin_difference
            self.data["sstskin_stddev"].fdata[mask] = self.data["sstfnd_stddev"].fdata[mask]
            self.data["sstskin_count"].fdata[mask] = self.data["sstfnd_count"].fdata[mask]
            self.data["sstskin"].fdata[~mask] = missing_value
        ...
        # Using sstskin, so calculate it from sstfnd.
        elif runParams.sst_gradients_switch == 0 and (self._input_data_provided("sstskin") == True) and (
                self._input_data_provided("sstfnd") == False):
            print(
                "%s SST gradient handling is off, using SSTskin to derive SSTfnd (SSTfnd = SSTskin + %f) for flux calculation (ignoring SSTfnd data in configuration file)." % (
                    function, runParams.cool_skin_difference));
            ...
            mask = self.data["sstskin"].fdata != missing_value

            self.data["sstfnd"].fdata[mask] = self.data["sstskin"].fdata[mask] + runParams.cool_skin_difference
            self.data["sstskin"].fdata[mask] = self.data["sstfnd"].fdata[mask]  # overwrite sstskin with updated sstfnd
            self.data["sstfnd_stddev"].fdata[mask] = self.data["sstskin_stddev"].fdata[mask]
            self.data["sstfnd_count"].fdata[mask] = self.data["sstskin_count"].fdata[mask]

            self.data["sstfnd"].fdata[~mask] = missing_value
            self.data["sstskin"].fdata[~mask] = missing_value

            # for i in arange(nx * ny):
            #     if self.data["sstskin"].fdata[i] != missing_value:
            #
            #         self.data["sstfnd"].fdata[i] = self.data["sstskin"].fdata[
            #                                            i] + runParams.cool_skin_difference  # calculate sstfnd from sstskin for use in bulk equation
            #         self.data["sstskin"].fdata[i] = self.data["sstfnd"].fdata[
            #             i]  # Copy sstfnd over sstskin to prevent accidental use of sst_gradients
        ...
        elif runParams.sst_gradients_switch == 1 and (self._input_data_provided("sstskin") == True) and (
                self._input_data_provided("sstfnd") == False):
            print(
                "%s SST gradient handling is on, using SSTskin and SSTfnd = SSTskin + %f for flux calculation (ignoring SSTfnd data in configuration file)." % (
                    function, runParams.cool_skin_difference));
            # setting sstfnd_ data fields to skin values
            mask = self.data["sstskin"].fdata != missing_value
            ...
            print(
                "\n%s sst_gradients_switch (%s), sstskin provided (%s), sstfnd provided (%s) combination not recognised, exiting." % (
                    function, runParams.sst_gradients_switch == 1, self._input_data_provided("sstskin"),
                    self._input_data_provided("sstfnd")))
            return 1;

        # If there is no pco2_sst data, we need to get it / generate it.
        if "pco2_sst" in self.data:
            print('Check that supplied pco2_sst is in degrees Celsius as specified in documentation')
        if "pco2_sst" not in self.data:  # SOCATv4
            try:
                print(
                    "No pco2_sst data were supplied. sstfnd will be used instead - make sure sstfnd is in Kelvin as specified in documentation.")  # DJF: 18/11/2024 - Added last part to check sstfnd is in Kelvin.
        ...
        self.data["sstskinC"].fdata[mask] = self.data["sstskin"].fdata[mask] - 273.15
        self.data["sstfndC"].fdata[mask] = self.data["sstfnd"].fdata[mask] - 273.15
        ...
        if (runParams.bias_sstskin_due_rain_switch == 1):
            add_sst_rain_bias(self.data["sstskin"].fdata, runParams.bias_sstskin_due_rain_value,
                              runParams.bias_sstskin_due_rain_intensity, self.data["rain"].fdata,
                              runParams.bias_sstskin_due_rain_wind, self.data["windu10"].fdata, nx * ny)

        # quality filtering of wind and Hs data
        # DJF - 18/11/2024 - Removed hardcoding of limits, this should be covered by min/maxBounds
        # for i in arange(nx * ny):
        #    if (self.data["windu10"].fdata[i] != missing_value):
        ...
        # DJF - 18/11/2024 - Removed this hardcoding of limits, this should be covered by min/maxBounds. Added statement for "pco2_sst" to be in degC above
        ...
        #    for i in arange(nx * ny):
        #       if np.isnan(self.data["pco2_sst"].fdata[i]) != True:# and self.data["pco2_sst"].fdata[i] > 0.0 ): #SOCATv4_IGA
        ...
        #
        #          if self.data["pco2_sst"].fdata[i] > 50.0 or self.data["pco2_sst"].fdata[i] < -3.0: # DJF - 14/11/2024 - Updated the limits, as this causes issues when temperatures are above 30.5 degC (e.g occur in Red Sea) or below -1.8degC (e.g. can occur in model output with high salinity, so freezing temp is closer to -2.0 degC)
        ...
        mask = (self.data["sstfnd"].fdata == self.data["sstfnd"].fillValue) | (
                self.data["sstskin"].fdata == self.data["sstskin"].fillValue)

        self.data["sstfnd"].fdata[mask] = missing_value
        self.data["sstskin"].fdata[mask] = missing_value

        # for i in arange(nx * ny):
        #     if ((self.data["sstfnd"].fdata[i] == self.data["sstfnd"].fillValue) or (
        #             self.data["sstskin"].fdata[i] == self.data["sstskin"].fillValue)):
        #         self.data["sstfnd"].fdata[i] = missing_value
        #         self.data["sstskin"].fdata[i] = missing_value
        ...
        if runParams.TAKAHASHI_DRIVER != True:
            if npany(self.data["pressure"].fdata > 10000.0):
                mask = self.data["pressure"].fdata != missing_value
                self.data["pressure"].fdata[mask] *= 0.01
        ...
        DeltaT_fdata = np.full(nx * ny, missing_value)
        ...
        if runParams.flux_calc == 1:
            ...
            mask = (self.data["sstfndC"].fdata != missing_value) & (self.data["sstskinC"].fdata != missing_value)
            DeltaT_fdata[mask] = self.data["sstfndC"].fdata[mask] - self.data["sstskinC"].fdata[mask]
            DeltaT_fdata[~mask] = missing_value

            # for i in arange(nx * ny):
            #     if ((self.data["sstfndC"].fdata[i] != missing_value) & (
            #             self.data["sstskinC"].fdata[i] != missing_value)):
            ...
            print(
                "%s Note: this assumes that the SSTskin dataset is the only temperature dataset, and so this is what will be used to calculate k and the solubility" % (
                    function))
        else:
        ...
        if runParams.schmidt_parameterisation == "schmidt_Wanninkhof2014":
            ...
            raise ValueError(
                "Unrecognised schmidt/solubility parameterisation selected: " + runParams.schmidtParameterisation);

        # Calculate solubility
        if runParams.schmidt_parameterisation == "schmidt_Wanninkhof2014":
            # calculating the skin solubility, using skin sst and salinity
            self.data["solubility_skin"].fdata = solubility_Wanninkhof2014(self.data["sstskin"].fdata,
                                                                           self.data["salinity_skin"].fdata,
                                                                           DeltaT_fdata, nx, ny, True,
                                                                           runParams.GAS.lower());
            # calculating the interfacial solubility
            self.data["solubility_fnd"].fdata = solubility_Wanninkhof2014(self.data["sstfnd"].fdata,
                                                                          self.data["salinity"].fdata, DeltaT_fdata, nx,
                                                                          ny, runParams.flux_calc,
                                                                          runParams.GAS.lower());
        elif runParams.schmidt_parameterisation == "schmidt_Wanninkhof1992":
            # calculating the skin solubility, using skin sst and salinity
            self.data["solubility_skin"].fdata = solubility_Wanninkhof1992(self.data["sstskin"].fdata,
                                                                           self.data["salinity_skin"].fdata,
                                                                           DeltaT_fdata, nx, ny, True,
                                                                           runParams.GAS.lower());
            # calculating the interfacial solubility
            self.data["solubility_fnd"].fdata = solubility_Wanninkhof1992(self.data["sstfnd"].fdata,
                                                                          self.data["salinity"].fdata, DeltaT_fdata, nx,
                                                                          ny, runParams.flux_calc,
                                                                          runParams.GAS.lower());
        else:
        ...
        #   pH2O                      #
        ###############################
        mask = (self.data["salinity_skin"].fdata != missing_value) & (self.data["sstskin"].fdata != missing_value)

        self.data["pH2O"].fdata[mask] = 1013.25 * np.exp(
            24.4543
            - (67.4509 * (100.0 / self.data["sstskin"].fdata[mask]))
            - (4.8489 * np.log(self.data["sstskin"].fdata[mask] / 100.0))
            - 0.000544 * self.data["salinity_skin"].fdata[mask]
        )
        ...
        #         self.data["pH2O"].fdata[i] = 1013.25 * np.exp(
        #             24.4543 - (67.4509 * (100.0 / self.data["sstskin"].fdata[i])) - (
        #                         4.8489 * np.log(self.data["sstskin"].fdata[i] / 100.0)) - 0.000544 *
        #             self.data["salinity_skin"].fdata[i])
        #     else:
        ...
        if "pgas_sw" in self.data:  # Only calculate partial pressure data is available
            ...
            #     if ((self.data["salinity_skin"].fdata[i] != missing_value) and (
            #             self.data["sstskin"].fdata[i] != missing_value) and (
            #             self.data["pressure"].fdata[i] != missing_value) and (
            #             self.data["sstfnd"].fdata[i] != missing_value) and (
            #             self.data["pco2_sst"].fdata[i] != missing_value) and (
            #             self.data["pgas_sw"].fdata[i] != missing_value) and (self.data["sstskin"].fdata[i] != 0.0)):
            #         if runParams.GAS == 'CO2' and runParams.pco2_data_selection != 3:
            ...
            #             self.data["pgas_sw_cor"].fdata[i] = pco2_increment + (self.data["pgas_sw"].fdata[i] * np.exp(
            #                 (0.0423 * (self.data["sstfndC"].fdata[i] - self.data["pco2_sst"].fdata[i])) - (0.0000435 * (
            #                             (self.data["sstfndC"].fdata[i] * self.data["sstfndC"].fdata[i]) - (
            #                                 self.data["pco2_sst"].fdata[i] * self.data["pco2_sst"].fdata[
            #                             i]))) + pCO2_salinity_term));
            #         else:
        ...
        if ("vgas_air" in self.data) and ("pgas_air" not in self.data):
            ...
            #             self.data["sstskin"].fdata[i] != missing_value) and (
            #             self.data["pressure"].fdata[i] != missing_value) and (
            #             self.data["vgas_air"].fdata[i] != missing_value) and (
            #             self.data["sstfnd"].fdata[i] != missing_value) and (
            #             self.data["pco2_sst"].fdata[i] != missing_value) and (
            #             self.data["pgas_sw"].fdata[i] != missing_value) and (self.data["sstskin"].fdata[i] != 0.0)):
            #         if runParams.GAS == 'CO2' and runParams.ATMGAS == 'V':
            #             self.data["pgas_air"].fdata[i] = (self.data["vgas_air"].fdata[i] * (
            #                         self.data["pressure"].fdata[i] - self.data["pH2O"].fdata[i])) / 1013.25;
            #         else:
            #             self.data["pgas_air"].fdata[i] = self.data["vgas_air"].fdata[i]
        ...
        if runParams.TAKAHASHI_DRIVER == False:  # Different for takahashi run to maintain compatability with verification run. This will be updated when verification runs are updated
            ...
            #         if runParams.GAS == 'CO2' and runParams.ATMGAS == 'V':
            #             # THtodo: 1e-6 can be removed...
            #             self.data["pgas_air_cor"].fdata[i] = (self.data["vgas_air"].fdata[i] * 1e-6 * (
            #                         self.data["pressure"].fdata[i] - self.data["pH2O"].fdata[i]) / (1e-6 * 1013.25)) + (
            #                                                      pco2_increment_air)
            #         else:
            #             self.data["pgas_air_cor"].fdata[i] = self.data["vgas_air"].fdata[i]

        # DJF 04/12/2024: Fixed indentation issue which causes this to never run even if the 'socat_fco2' pco2_data_selection, and therefore atmospheric side is pCO2 and not fCO2.
        ...
        if runParams.pco2_data_selection == 2 or runParams.pco2_data_selection == 4 or runParams.pco2_data_selection == 45:
            ...
            b11 = -1636.75 + (12.0408 * self.data["sstskin"].fdata[mask]) - (
                    0.0327957 * self.data["sstskin"].fdata[mask] ** 2) + (
                          3.16528e-5 * self.data["sstskin"].fdata[mask] ** 3)
            d12 = 57.7 - (0.118 * self.data["sstskin"].fdata[mask])
            ...
            #         b11_fdata[i] = -1636.75 + (12.0408 * self.data["sstskin"].fdata[i]) - (
            #                     0.0327957 * self.data["sstskin"].fdata[i] * self.data["sstskin"].fdata[i]) + (
            #                                    3.16528e-5 * self.data["sstskin"].fdata[i] * self.data["sstskin"].fdata[
            #                                i] * self.data["sstskin"].fdata[i])
            #         d12_fdata[i] = 57.7 - (0.118 * self.data["sstskin"].fdata[i])
            #         # gas constant
            #         # R = 82.0578 # in [cm^3 atm/(mol K)]
            #         R = 83.1451  # DJF 11/12/2024: Updated gas molar constant to CODATA2022 - https://physics.nist.gov/cgi-bin/cuu/Value?r|search_for=gas+constant
            #         # DJF 11/12/2024: Above gas molar constant is consistent with pyCO2sys v1.8.3.3
            #
            #         # LEGACY: 1/0.987 = 1.0131712 - conversion between bar and atm, so *1013.25 is the conversion from millibar to atm.
            #         # LEGACY: the combination of the B11 and d12 terms are in cm^3/mol and so these cancel with the P/RT term (in mol/cm^3) so the whole of the exp term is dimensionless
            #         # DJF 11/12/2024: Assessed consistency with PyCO2sys v1.8.3.3 - Updated the pressure section. Pressure (mb) / 1013.25 to convert to atm. pressure (atm) / 0.98692 to convert to bar.
            #         # DJF 11/12/2024: pCO2(atm) (unchanged) and fCO2(atm) (changed for FEv4.0.9.1) were consistent with PyCO2sys to within 1e-14 and 1e-6 uatm respectively.
            #         self.data["pgas_air_cor"].fdata[i] = self.data["pgas_air_cor"].fdata[i] * np.exp(
            #             (b11_fdata[i] + (2 * d12_fdata[i])) * (
            #                         ((self.data["pressure"].fdata[i] / 1013.25) / 0.98692) / (
            #                             R * self.data["sstskin"].fdata[i])))
        ...
        if runParams.TAKAHASHI_DRIVER:  # Assumes CO2 data input is not suppled as concentrations
            ...
            #     if self.data["pgas_air"].fdata[i] != missing_value and self.data["pH2O"].fdata[i] != missing_value and \
            #             self.data["pressure"].fdata[i] != missing_value and self.data["vgas_air"].fdata[
            #         i] != missing_value:
            #         pH2O_takahashi_fdata[i] = self.data["pressure"].fdata[i] - (
            #                     self.data["pgas_air"].fdata[i] * 1e-6 * 1013.25) / (
            #                                               self.data["vgas_air"].fdata[i] * 1e-6)
        ...
        for kParameterisationFunctor in self.kParameterisationFunctors:
            ...
            for inputDataName in kParameterisationFunctor.input_names():
                if inputDataName not in self.data:  # This additional check isn't really needed as it is done in the functor and in the driver script.
                    raise KeyError(
                        "Selected kParameterisation (" + kParameterisationFunctor.name + ") requires input data layers which have not been provided. Required the following DataLayers:\n" + str(
                            kParameterisationFunctor.input_names()));
            ...
            if kParameterisationOutput == False:
                raise RuntimeError(
                    "%s: k parameterisation component (%s) returned False indicating k has not been calculated successfully." % (
                        function, kParameterisationFunctor.name));
        ...
        if (runParams.bias_k_switch == 1):
            add_bias_k_biology_wind(self.data["k"].fdata, runParams.bias_k_value, self.data["biology"].fdata,
                                    runParams.bias_k_biology_value, self.data["windu10"].fdata,
                                    runParams.bias_k_wind_value, nx * ny, runParams.bias_k_percent_switch)
            if runParams.bias_k_percent_switch == 0:
                print(
                    "\n%s Adding bias to chosen k (k_fdata) parameterisation data (bias value of %lf ms^-1 added, where biology (biology fdata) is > %lf mg m^-3 and wind speed (windu10) is < %lf m s^-1)" % (
                        function, runParams.bias_k_value, runParams.bias_k_biology_value, runParams.bias_k_wind_value))
            else:
                print(
                    "\n%s Adding bias to chosen k (k_fdata) parameterisation data (bias value of - %lf percent being used, where biology (biology fdata) is > %lf mg m^-3 and wind speed (windu10) is < %lf m s^-1)" % (
                        function, runParams.bias_k_value, runParams.bias_k_biology_value, runParams.bias_k_wind_value))
        ...
        if runParams.rain_wet_deposition_switch:
            ...
            self.data["solubility_distilled"].fdata = calculate_solubility_distilled(self.data["salinity"].fdata,
                                                                                     runParams.rain_wet_deposition_switch,
                                                                                     self.data["sstskin"].fdata,
        ...
        ###################################################
        outputUnitMolecularMass = get_output_unit_molecular_mass(
            runParams.GAS);  # Different gases mean the output units will be different. Thus different molecular masses are needed to correctly calculate moles.
        ...
        ##############################
        mask = (
                (self.data["k"].fdata != missing_value) &
                (self.data["concw"].fdata != missing_value) &
                (self.data["conca"].fdata != missing_value)
        )
        ...
        if runParams.flux_calc in [1, 2]:
            ...
            raise ValueError(
                "Unrecognised flux calculation. Currently supported options are 'rapid' (1), 'equilibrium' (2) and 'bulk' (3). Recieved: " + str(
                    runParams.flux_calc)
            )

        if runParams.rain_wet_deposition_switch:
            if "pgas_air_cor" not in self.data:
                raise ValueError(
                ...
                invalid_FH06_mask = mask & ~valid_FH06_mask

                self.data["FH06"].fdata[valid_FH06_mask] += self.data["FKo07"].fdata[valid_FH06_mask]
                self.data["FH06"].fdata[invalid_FH06_mask] = self.data["FKo07"].fdata[invalid_FH06_mask]

        # Set missing_value where mask is False
        self.data["FH06"].fdata[~mask] = missing_value
        #
        # for i in arange(nx * ny):
        #     if ((self.data["k"].fdata[i] != missing_value) and (self.data["concw"].fdata[i] != missing_value) and (
        ...
        #                 self.data["FH06"].fdata[i] = (kd_component * (
        #                             self.data["concw"].fdata[i] - self.data["conca"].fdata[i])) + (kb_component * (
        #                             self.data["concw"].fdata[i] - (
        #                                 runParams.kb_asymmetry * self.data["conca"].fdata[i])))
        #             else:
        #                 self.data["FH06"].fdata[i] = (self.data["k"].fdata[i] * k_factor) * (
        #                             self.data["concw"].fdata[i] - self.data["conca"].fdata[i])
        #
        ...
        #                 self.data["FKo07"].fdata[i] = -(self.data["rain"].fdata[i] * (24.0 / 1000.0)) * (
        #                             concFactor * self.data["solubility_distilled"].fdata[i]) * \
        #                                               self.data["pgas_air_cor"].fdata[i]
        ...
        if runParams.TAKAHASHI_DRIVER == True:
            solskin_takadata = np.full(nx * ny, missing_value)
            FH06_takadata = np.full(nx * ny, missing_value)
            mask_sol = self.data["solubility_skin"].fdata != missing_value
            solskin_takadata[mask_sol] = self.data["solubility_skin"].fdata[mask_sol] * 1000

            mask_FH06 = self.data["FH06"].fdata != missing_value
            FH06_takadata[mask_FH06] = self.data["FH06"].fdata[mask_FH06] / 30.5

            # for i in arange(nx * ny):
            #     if self.data["solubility_skin"].fdata[i] != missing_value:
            #         solskin_takadata[i] = self.data["solubility_skin"].fdata[
            #                                   i] * 1000  # from 'mol kg-1 atm-1' to 'mmol kg-1 atm-1'
            #     if self.data["FH06"].fdata[i] != missing_value:
        ...
        if "pgas_sw_cor" in self.data and "pgas_air_cor" in self.data:
            ...
            #             self.data["pgas_air_cor"].fdata[i] != missing_value)):
            #         self.data["dpco2_cor"].fdata[i] = (
            #                     self.data["pgas_sw_cor"].fdata[i] - self.data["pgas_air_cor"].fdata[i])
            #     else:
        ...
        if runParams.TAKAHASHI_DRIVER != True:
            if (self.runParams.pco2_data_selection == 0):
                self.data["pgas_sw"].fdata = np.full(self.nx * self.ny, missing_value)
                self.data["pgas_sw_stddev"].fdata = np.full(self.nx * self.ny, missing_value)
                ...
                try:
                    for inputDataLayer in piFunctor.input_names():  # Check all required inputs exist.
                        if inputDataLayer not in self.data:
                            raise ValueError("%s: Missing input DataLayer (%s) for process indicator functor %s." % (
                                function, inputDataLayer, piFunctor.name));
